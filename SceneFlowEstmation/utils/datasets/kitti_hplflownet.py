...
class Kitti(SceneFlowDataset):
    def __init__(self, root_dir, nb_points, mode):
        ...
        # ind_val = set(np.sort(np.linspace(0, 141, 140).astype("int")))
        # ind_all = set(np.sort(np.arange(142).astype("int")))
        ind_val = set(np.linspace(0, 141, 70).astype("int"))
        ind_all = set(np.arange(142).astype("int"))
        ...
        if self.mode == "train":
            ...
            self.paths = filenames[list(ind_all)]
