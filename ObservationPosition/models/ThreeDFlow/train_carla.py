...
def main():
    ...
    os.environ['CUDA_VISIBLE_DEVICES'] = args.gpu if args.multi_gpu is None else '1,2,3'

    '''CREATE DIR'''
    experiment_dir = Path('./experiment/')
    experiment_dir.mkdir(exist_ok=True)
    file_dir = Path(str(experiment_dir) + '/ThreeDFlow%sFlyingthings3d-'%args.model_name + str(datetime.datetime.now().strftime('%Y-%m-%d_%H-%M')))
...
def eval_sceneflow(model, loader):
    ...
    for batch_id, data in tqdm(enumerate(loader), total=len(loader), smoothing=0.9):
        ...
        with torch.no_grad():
            ...
            eval_loss = multiScaleLoss(pred_flows, gt_flows)

            epe3d = torch.norm(pred_flows[0].permute(0, 2, 1) - gt_flows[0].permute(0,2,1), dim = 2).mean()
