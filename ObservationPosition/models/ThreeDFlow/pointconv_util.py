...
from models.ThreeDFlow.pointnet23dflow import pointnet2_utils
...
class Conv2d(nn.Module):
    ...
    def forward(self,x):
        # x (b,n,s,c)
        x = x.permute(0,3,2,1).to(torch.float32) #(b,c,s,n)
...
def square_distance(src, dst):
    ...
    B, N, _ = src.shape
    _, M, _ = dst.shape
    dist = -2 * torch.matmul(src.to(torch.float32), dst.permute(0, 2, 1).to(torch.float32))
...
def sample_and_group( npoint, radius, nsample, xyz, xyz_raw, label, points, knn=True, use_xyz=True, use_fps=True):
    ...
    if npoint == 2048 and not use_fps:
        # new_xyz = new_xyz[0]#torch.cat(new_xyz, dim=0)
        # new_xyz_raw = new_xyz_raw[0]#torch.cat(new_xyz_raw, dim=0)
        # new_label= new_label[0]#torch.cat(new_label, dim=0)
        new_xyz = torch.split(xyz, 2048, 1)  # (batch_size, 2048, 3)
        new_xyz_raw = torch.split(xyz_raw, 2048, 1)  # (batch_size, 2048, 3)
        new_label = torch.split(label, 2048, 1)   # (batch_size, 2048, 3)
    elif npoint == 8192 and not use_fps:
        new_xyz = xyz # (batch_size, 2048, 3)
        new_xyz_raw = xyz_raw  # (batch_size, 2048, 3)
        new_label = label   # (batch_size, 2048, 3)
    else:
...
class PointnetFpModule(nn.Module):
    ...
    def forward(self, xyz1, xyz2, points1, points2):
        ...
        """
        xyz1 = xyz1.contiguous().to(torch.float32)
        xyz2 = xyz2.contiguous().to(torch.float32)
