...
from models.ThreeDFlow.pointconv_util import (All2AllCostVolume, Conv1d, CostVolume,
                            FlowPredictor, PointnetFpModule, PointNetSaModule,
                            SetUpconvModule, WarpingLayers)

scale = 1.0

class ThreeDFlow(nn.Module):
    def __init__(self,  is_training, bn_decay=None):
        super(ThreeDFlow, self).__init__()
        RADIUS1 = 0.5
        RADIUS2 = 1.0
        RADIUS3 = 2.0
        RADIUS4 = 4.0

        self.layer0 = PointNetSaModule(npoint=8192, radius=RADIUS1, nsample=32, in_channels=3,mlp=[16,16,32],mlp2=None, group_all=False, is_training=is_training, bn_decay=bn_decay, use_fps=False)
        
        self.layer1 = PointNetSaModule(  npoint=1024, radius=RADIUS1, nsample=24, in_channels=32,mlp=[32,32,64], mlp2=None, group_all=False, is_training=is_training, bn_decay=bn_decay)
        ...
        self.flow_pred4 = FlowPredictor([32,64,32,3],mlp=[128,64,64], is_training = is_training , bn_decay = bn_decay, npoint=8192)
    ...
    def forward(self, xyz1,xyz2,color1,color2,label):
        ...
        return l0_xyz_f1, l0_xyz_f2, l0_points_f1, l0_points_f2
...
if __name__ == '__main__':
    ...
    model = ThreeDFlow(False).cuda()
    # print(model)
    # output = model(input,input,input,input,input)
    ...
    traced_model = torch.jit.trace(model, (dump_input, dump_input, dump_input, dump_input, dump_input))
    timer = 0

    timer = 0
    for i in range(100):
        t = time.time()
        _ = traced_model(input,input,input,input,input)
        timer += time.time() - t
    print(timer / 100.0)
