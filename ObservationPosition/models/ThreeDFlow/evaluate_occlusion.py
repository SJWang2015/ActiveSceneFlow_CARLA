...
def main():
    ...
    val_dataset = FT3D(root_dir=args.data_root, nb_points=args.num_points, mode="test")
    ...
    for i, data in tqdm(enumerate(val_loader, 0), total=len(val_loader), smoothing=0.9):
        ...
        norm1 = data['sequence'][2]
        norm2 = data['sequence'][3]
        mask = data['ground_truth'][0]
        # mask1 = torch.ones([pc1.shape[0], pc1.shape[1]]).contiguous().cuda()
        flow = data['ground_truth'][1]

        # move to cuda
        pos1 = pos1.cuda().float().squeeze(1)
        pos2 = pos2.cuda().float().squeeze(1)
        norm1 = norm1.cuda().float().squeeze(1)
        norm2 = norm2.cuda().float().squeeze(1)
        flow = flow.cuda().float().squeeze(1)
        mask = mask.unsqueeze(2).cuda().float().squeeze(1).squeeze(1)

        model = model.eval()
        with torch.no_grad():
            pred_flows, gt_flows, pc1, pc2, raw_pc1, raw_pc2, _mask = model(
                pos1, pos2, pos1, pos2, flow, mask
            )
